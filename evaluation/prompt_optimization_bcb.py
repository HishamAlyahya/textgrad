...
def config():
    ...
    parser.add_argument("--evaluation_engine", type=str, default="gpt-4-turbo", help="The API to use for evaluation.")
...
def eval_code_sample(item, eval_fn, model):
    # print(item)
    prompt, tests, entry_point = item
    prompt = tg.Variable(prompt, requires_grad=False, role_description="query to the language model")
    tests = tg.Variable(tests, requires_grad=False, role_description="tests for the code")
    entry_point = tg.Variable(entry_point, requires_grad=False, role_description="entry point for the code")
    ...
    # return int(res["base"][0] == "pass")
    return eval_fn(inputs=dict(output=response, tests=tests, entry_point=entry_point))
    # return eval_fn(output=response.value, task=task)
...
def eval_dataset(test_set, eval_fn, model, max_samples: int=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_threads) as executor:
        ...
        for _, sample in enumerate(test_set):
            ...
            future = executor.submit(eval_code_sample, sample, eval_fn, model)
        ...
        for future in tqdm_loader:
            ...
            accuracy_list.append(int(acc_item.value))
...
# llm_api_test = tg.get_engine(engine_name=args.test_engine, base_url="https://openrouter.ai/api/v1")
...
for epoch in range(1):
    for steps, (batch_prompts, batch_tests, batch_entry_points) in enumerate((pbar := tqdm(train_loader, position=0))):

        pbar.set_description(f"Training step {steps}. Epoch {epoch}")
        optimizer.zero_grad()
        losses = []
        for (prompt, tests, entry_point) in zip(batch_prompts, batch_tests, batch_entry_points):
            # x = tg.Variable(x, requires_grad=False, role_description="query to the language model")
            # y = tg.Variable(y, requires_grad=False, role_description="correct answer for the query")
            prompt = tg.Variable(prompt, requires_grad=False, role_description="query to the language model")
            tests = tg.Variable(tests, requires_grad=False, role_description="tests for the code")
            ...
            # except:
            #     eval_output_variable = eval_fn([x, y, response])
            eval_output_variable = eval_fn(inputs=dict(output=response, tests=tests, entry_point=entry_point))
...
with open(f"./results/results_{args.task}_{args.test_engine.split('/')[-1]}.json", "w") as f:
    json.dump(results, f)